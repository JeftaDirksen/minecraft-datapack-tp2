...
scoreboard players add @a[scores={tp-home=1..}] tp-home-set 0
execute as @a[scores={tp-home=1..,tp-home-set=1,tp-cooldown=1..}] run tellraw @s ["",{text:"Can't do that just yet, wait ",color:"red"},{score:{name:"@s",objective:"tp-cooldown"},color:"red"},{text:" sec.",color:"red"}]
tellraw @a[scores={tp-home=1..,tp-home-set=0}] ["",{text:"No home set",color:"red"}]
execute as @a[scores={tp-home=1..,tp-home-set=1,tp-cooldown=0}] run function tp:home
scoreboard players reset @a[scores={tp-home=1..}] tp-home

# Back trigger
scoreboard players enable @a tp-back
scoreboard players add @a[scores={tp-back=1..}] tp-back-set 0
execute as @a[scores={tp-back=1..,tp-back-set=1,tp-cooldown=1..}] run tellraw @s ["",{text:"Can't do that just yet, wait ",color:"red"},{score:{name:"@s",objective:"tp-cooldown"},color:"red"},{text:" sec.",color:"red"}]
tellraw @a[scores={tp-back=1..,tp-back-set=0}] ["",{text:"Nowhere to go back to",color:"red"}]
execute as @a[scores={tp-back=1..,tp-back-set=1,tp-cooldown=0}] run function tp:back
